...
/**
 * Class ComSiteAccountContronller
 *
 * 公司前台验证控制器 by cooper
 */
class ComSiteAccountContronller extends ComSiteContronller {
    ...
    {
        return View::make('com/site/account/login')->with('com', $com);
    }
    ...
    public function postLogin($com){
        ...
        if ($validator->passes())
        {
            ...
            {
                // 验证是否为后台用户登入前台
                $admin_user_id = DB::table('admin_user')->where('user_id', Auth::user()->id)->pluck('user_id');
                if ($admin_user_id) {
                    return Redirect::to($com->path . '/login')->with('error', '邮箱 / 密码 无效');
                }
                
                // Redirect to the users page.
                //
                // return Redirect::to($com->path)->with('success', '登陆成功');
                return Redirect::to($com->path)->with('success');
            }
            else
            {
        ...
        // 返回登录界面，并提示错误信息
        //
        return Redirect::to($com->path . '/login')->withInput(Input::all())->withErrors($validator->getMessageBag());
    }
    ...
    public function getLogout($com){
        ...
        // Redirect to the users page.
        //
        // return Redirect::to($com->path . '/')->with('success', '退出成功');
        return Redirect::to($com->path . '/')->with('success');
    }
    ...
    public function getIndex($com)
    {
        ...
        // 加载模板
        //
        return View::make('com/site/account/index')->with('com', $com)->with('user', Auth::user());
    }
    ...
    public function postIndex($com)
    {
        ...
        if ($validator->passes())
        {
            ...
            if (Input::get('password') !== '')
            {
                ...
                // 转跳登录页面
                //
                return Redirect::to($com->path . '/login')->with('success', '密码修改、请重新登录！');
            }

            // 提示修改信息
            //
            return Redirect::to($com->path . '/account')->with('success', '资料更新成功!');
        }
