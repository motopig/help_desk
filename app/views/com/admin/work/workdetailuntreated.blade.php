...
<ol class="breadcrumb">
    ...
    <li class="dropdown active">
        ...
        <li><a href="{{ URL::to($com->path.'/admin/work/untreated/g') }}">组员工单</a></li>
        <li class="divider"></li>
        <li><a href="{{ URL::to($com->path.'/admin/work/untreated/m') }}">我的工单</a></li>
        </ul>
    </li>
    @else
    <li><a href="{{ URL::to($com->path.'/admin/work/untreated/'.$level) }}">处理中工单</a></li>
    @endif
    ...
    <ul class="media-list">
        ...
        <li class="media">
          <span class="pull-left" title="{{$workerOrder['0']->firm}}">
            ...
            <a href="javascript:void(0)" onclick="askAnchor('{{$workerOrder[0]->name}}','{{$con->created_at}}')">
        ...
        @elseif (isset($con->reply))
        @if ($con->system == 1)
        <li class="media">
          <a href="http://www.no" target="_blank" onFocus="this.blur()" class="pull-right">
        ...
        @elseif ($con->system == 2)
        <li class="media">
          <span class="pull-right" title="{{$con->position}}">
          ...
          <div class="media-body pull-right">
            ...
            {{$con->reply}}
                ...
                <span class="glyphicon glyphicon-eye-close" title="私有记录，客户不可见"></span>
                @if ($con->reply_anchor != '')
                <a href="#{{$con->reply_anchor}}">
                    <span class="glyphicon glyphicon-send" id="{{$con->id}}ask" onmouseover="toolTip({{$con->id}})" 
                        data-placement="bottom" data-original-title="{{$con->reply_anchor}}">
                    </span>
                </a> 
                @endif
                @if ($con->enclosure_id != 0)
        ...
        <li class="media">
          ...
          <div class="media-body pull-right">
            ...
            {{$con->reply}}
                ...
                @if ($con->check == 1 && $con->check_status == 0)
                <span class="glyphicon glyphicon-ban-circle" title="审核后客户可见"></span>
                @elseif ($con->check == 1 && $con->check_status == 1)
                <span class="glyphicon glyphicon-remove-circle" title="此条回复审核未通过"></span>
                @endif
                @if ($con->reply_anchor != '')
                <a href="#{{$con->reply_anchor}}">
                    <span class="glyphicon glyphicon-send" id="{{$con->id}}ask" onmouseover="toolTip({{$con->id}})" 
...
<textarea name="reply" class="form-control text-danger" id="text-content" placeholder="点击发表评论"></textarea>
<div>
<!-- 回复隐藏可见配置 -->
<a href="javascript:eyes(2)"; class="add-attachment toolbar-item" onFocus="this.blur()" unselectable="on" 
tabindex="-1" style="position: relative; overflow: hidden; direction: ltr; float:right; left: -40px; top: -30px;">
<span class="glyphicon glyphicon-eye-open" id="eyest"></span>
</a>
<a href="javascript:eyes(0)"; class="add-attachment toolbar-item" onFocus="this.blur()" unselectable="on" 
tabindex="-1" style="position: relative; overflow: hidden; direction: ltr; float:right; left: -40px; top: -30px;">
<span class="glyphicon glyphicon-eye-close" style="display:none;" id="eyesf"></span>
</a>
<!-- 回复隐藏可见配置 -->
<a href="javascript:;" class="add-attachment toolbar-item" onFocus="this.blur()" unselectable="on" 
tabindex="-1" style="position: relative; overflow: hidden; direction: ltr; float:right; top: -50px;">
<br />
<span class="glyphicon glyphicon-paperclip" title="添加附件"></span>
...
<hr >
@if ($level == '4')
<a class="view-link" style="text-decoration:none;" href="{{ URL::to($com->path.'/admin/work/dispatch/'.$workerOrder['0']->id) }}">
<button class="btn btn-warning">工单邀请</button>
</a>
<a><button type="submit" class="btn btn-primary">工单回复</button></a>
<!-- binway需求：完成按钮客服没权限操作 -->
<!-- <a href="{{ URL::to($com->path.'/admin/work/workachieve/'.$workerOrder['0']->id) }}">
<button type="button" class="btn btn-success">工单完成</button>
</a> -->
@elseif ($level == '3')
<div class="btn-group dropup">
<button type="button" class="btn btn-warning dropdown-toggle" data-toggle="dropdown">
工单分派<span class="caret"></span>
</button>
<ul class="dropdown-menu" role="menu">
<li><a class="view-link" href="{{ URL::to($com->path.'/admin/work/dispatch/'.$workerOrder['0']->id) }}">工单邀请</a></li>
<li class="divider"></li>
<li><a class="view-link" href="{{ URL::to($com->path.'/admin/work/allot/'.$workerOrder['0']->id) }}">工单分配</a></li>
</ul>
</div>
<a><button type="submit" class="btn btn-primary">工单回复</button></a>
<a href="{{ URL::to($com->path.'/admin/work/workachieve/'.$workerOrder['0']->id) }}">
