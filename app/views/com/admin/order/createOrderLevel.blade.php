...
function askreply(type) {
	if (type == 1) {
        ...
        $("#reply_out_level_f").hide();
        $("#reply_out_level_t").show();
	} else if (type == 0) {
		$("#askuser").show();
		$("#replyuser").hide();

        $("#reply_out_level_t").hide();
        $("#reply_out_level_f").show();
	}
}
</script>
@stop
	...
	<div class="col-lg-12">
        ...
        <form class="col-sm-12" role="form" method="post" enctype="multipart/form-data" onsubmit="return check()">
            ...
            <div class="form-group clearfix {{{ $errors->has('admin') ? 'has-error' : '' }}}">
                ...
                <div class="col-sm-10" id="reply_out_level_t" style="display:none;">
                    @if ($admin->count() >= 1)
                ...
                <div class="col-sm-10" id="reply_out_level_f">
...
function check() {
    ...
    if ($('#checktype').val() == 'user' && $('#check_user_name').val() == '') {
        alert('请输入收件人!');
